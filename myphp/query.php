...
function main(){
    session_start();
    $connection=new mysqli("127.0.0.1","root","root","students1911a");//实例化一个MySQL链接
    if($connection->error){//判断链接是否成功
        die("connect error!");//如果失败，结束并输出一个提示信息
    }else{//如果成功执行一下信息 recordset
        if($rs=$connection->query("select * from students1911;")){//$connection->query("select * from students;") 返回记录集
            //query方法可用于查询更新数据库信息
            echo "{code:2000,list:[";
            while($row=$rs->fetch_assoc()){//循环遍历数据库中表内的数据，每次循环读出一条数据并赋值给$row变量，一条信息就是表中的一行信息
                echo "{ID:".$row["ID"].",name:'".$row["s_name"]."',sex:".$row["s_sex"].",phone:'".$row["s_phone"]."',score:'".$row["s_score"]."',seat:'".$row["s_seat"]."',profession:'".$row["s_profession"]."'},";
            }
            echo "]}";
        }
